...
#     temp1 = np.exp(x)
#     temp2 = np.exp(-x)
#     return 4 / np.power(temp1 + temp2, 2)
...
class Network:
    ...
    def feed_forward(self, x, y):
        ...
        self.loss = np.mean(-(np.sum(self.y*np.log(self.a[-1])+(1-self.y)*np.log(1-self.a[-1]), axis=0)))
    ...
    def back_propagate(self):
        ...
        for k in range(self.batch_size):
            for i in range(self.num_layers):
                if i == 0:
                    # self.dz[-1][:,k] = 2*(self.a[-1][:, k]-self.y[:,k])*tanh_1(self.z[-1][:,k])
                    self.dz[-1][:,k] = -(self.y[:,k]/self.a[-1][:,k]-(1-self.y[:,k])/(1-self.a[-1][:,k]))*sigmoid_1(self.z[-1][:,k])
                else:

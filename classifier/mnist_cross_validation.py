...
def load_mnist():
    images = np.load('/Python/Machine Learning/mnist_images.npy')
    ...
    labels = np.load('/Python/Machine Learning/mnist_labels.npy')
...
def main(num_split, max_depth):
    ...
    for num in range(20, 110, 10):
        images_pca = mypca(images, num)
        print('------useing component: {0}------'.format(num))
        image_blocks, label_blocks = create_blocks(images_pca, labels, num_blocks=num_split)
        filename = 'log_pca({0}).txt'.format(num)
        ...
        f.close()
        np.save('log_avg_{0}_component({1}).npy'.format(max_depth, num), avg_accuracy)
        np.save('log_std_{0}_component({1}).npy'.format(max_depth, num), std_accuracy)
        np.save('log_num_{0}_component({1}).npy'.format(max_depth, num), num_estimators)


main(num_split=5, max_depth=10)


avg = np.load('log_avg_10_component(50).npy')
std = np.load('log_std_10_component(50).npy')
num = np.load('log_num_10_component(50).npy')

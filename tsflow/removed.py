...
conv1 = slim.conv2d(normalized_x,
                    num_outputs=16,
                    kernel_size=[3, 3],
                    #activation_fn=tf.nn.tanh,
                    weights_initializer=tf.constant_initializer(np.random.rand(1,3,3,16)*0.1),
...
conv2 = slim.conv2d(pool1,
                    ...
                    kernel_size=[3, 3],
                    #activation_fn=tf.nn.tanh,
                    weights_initializer=tf.constant_initializer(np.random.rand(16, 3, 3, 32)* 0.1),
                    biases_initializer=tf.constant_initializer(0.1)
                    )
...
fc = slim.fully_connected(flatten, 64)#, activation_fn=tf.nn.sigmoid)

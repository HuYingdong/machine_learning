...
def load_data4D():
    ...
    X_train = images[:3000, :, :].reshape((3000, 28, 28, 1)).astype(np.float32)
    X_test = images[3000:, :, :].reshape((57000, 28, 28, 1)).astype(np.float32)
...
conv1 = slim.conv2d(normalized_x, num_outputs=30, kernel_size=[5, 5], activation_fn=tf.nn.tanh)
print('conv1 shape:', conv1.get_shape())

pool1 = slim.max_pool2d(conv1, kernel_size=[2, 2])
print('pool1 shape:', pool1.get_shape())

conv2 = slim.conv2d(pool1, num_outputs=60, kernel_size=[5, 5], activation_fn=tf.nn.tanh)
